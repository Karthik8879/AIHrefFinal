...
export async function fetchAnalyticsSummary(
  ...
  const url = `${API_BASE_URL}/api/analytics/summary?siteId=${encodeURIComponent(
    siteId
  )}&range=${range}`;
...
export async function fetchRealTimeAnalytics(
  ...
  const url = `${API_BASE_URL}/api/analytics/realtime?siteId=${encodeURIComponent(
    siteId
  )}&range=${range}`;
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch real-time analytics: ${response.status} ${response.statusText}`
    );
  }
...
export function convertDailySnapshotToVisitorCount(
  snapshot: any
): DailyVisitorCount {
  try {
    console.log(
      "convertDailySnapshotToVisitorCount - Input snapshot:",
      snapshot
    );
    ...
    if (typeof snapshot.date === "string") {
      ...
      console.log(
        "convertDailySnapshotToVisitorCount - API format result:",
        result
      );
      return result;
    } else {
      ...
      const result = {
        date: new Date(parseInt(snapshot.date.$date.$numberLong))
          .toISOString()
          .split("T")[0],
      ...
      console.log(
        "convertDailySnapshotToVisitorCount - MongoDB format result:",
        result
      );
      return result;
    }
    ...
    console.error(
      "Error in convertDailySnapshotToVisitorCount:",
      error,
      "snapshot:",
      snapshot
    );
...
export function convertDailySnapshotsToVisitorCounts(
  snapshots: any[]
): DailyVisitorCount[] {
  ...
  return snapshot.topPages.map((page: any) => ({
    ...
    count:
      typeof page.count === "number"
        ? page.count
        : parseInt(page.count.$numberLong),
  }));
}

export function convertDailySnapshotToCountryCount(
  snapshot: any
): CountryCount[] {
  return snapshot.topCountries.map((country: any) => ({
    ...
    count:
      typeof country.count === "number"
        ? country.count
        : parseInt(country.count.$numberLong),
  }));
}
    ...
    const sortedSnapshots = snapshots.sort((a, b) => {
      ...
      const dateA =
        typeof a.date === "string"
          ? new Date(a.date)
          : new Date(parseInt(a.date.$date.$numberLong));
      const dateB =
        typeof b.date === "string"
          ? new Date(b.date)
          : new Date(parseInt(b.date.$date.$numberLong));
...
export async function fetchDailySnapshots(
  ...
  const url = `${API_BASE_URL}/api/analytics/daily-snapshots?siteId=${encodeURIComponent(
    siteId
  )}&range=${range}`;
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch daily snapshots: ${response.status} ${response.statusText}`
    );
  }
...
export async function fetchRawEvents(
  ...
  const url = `${API_BASE_URL}/api/analytics/raw-events?siteId=${encodeURIComponent(
    siteId
  )}&range=${range}`;
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch raw events: ${response.status} ${response.statusText}`
    );
  }
