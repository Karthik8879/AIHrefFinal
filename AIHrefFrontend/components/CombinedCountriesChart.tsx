...
interface CombinedCountriesChartProps {
  analytics: CombinedAnalytics;
}

export default function CombinedCountriesChart({
  analytics,
}: CombinedCountriesChartProps) {
  // Get top 10 countries from the combined analytics data
  ...
  const datasets = analytics.sites.map(site => {
    ...
    // For demonstration, we'll distribute country data proportionally
    // In a real implementation, you'd want actual site-specific country data
    const siteData = topCountries.map(country => {
      const siteRatio = site.visitors / analytics.totalVisitors;
      return Math.round(country.count * siteRatio);
    });
  ...
  return (
    <ChartContainer title="Visitors by Top 10 Countries (Combined)">
